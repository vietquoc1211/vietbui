...
@Component({
  templateUrl: './khai-bao-gia-tri-tuy-chon-create.component.html'
})
export class GiaTriTuyChonCreateComponent {
  ...
  @ViewChild('form') form;
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private dialogRef: MatDialogRef<GiaTriTuyChonCreateComponent>, public _data: DataService, private fb: FormBuilder) 
  {
    this.loaddata();
  }
  createform() {
    this.formdata = new FormGroup({
      ...
      Value: new FormControl('', [Validators.required, Validators.maxLength(500)]),
    });
  }
  setvalueform(values) {
    this.formdata.controls['Key'].setValue(values.Key);
    this.formdata.controls['Form'].setValue(values.Form);
    this.formdata.controls['Value'].setValue(values.Value);
  }
  loaddata() {
    this.createform();
  ...
  savedata(values) {
    ...
    this.const_data.Form = values.Form;
    this.const_data.Value = values.Value;
