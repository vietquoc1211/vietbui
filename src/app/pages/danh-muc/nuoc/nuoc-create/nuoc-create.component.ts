...
@Component({
  templateUrl: "./nuoc-create.component.html",
})
export class NuocCreateComponent {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,
    private dialogRef: MatDialogRef<NuocCreateComponent>,
  ...
  createform() {
    this.formdata = new FormGroup({
      TenNuoc: new FormControl('', [Validators.required, Validators.maxLength(256)]),
      Lock: new FormControl(false)
    });
  }
  setvalueform(values) {
    this.formdata.controls['TenNuoc'].setValue(values.TenNuoc);
  ...
  savedata(values) {
    ...
    this.issave = true;
    this.const_data.TenNuoc = values.TenNuoc;
